...
6. **Dynamic specialists (optional).** If the change enters a domain where you want
   ...
   deployment config, cryptography, api-contract/back-compat,
   multi-tenancy/authz-isolation, performance, accessibility, privacy/PII,
   license/dependency-provenance, concurrency, observability — illustrative, not
