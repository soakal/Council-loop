...
Report council status concisely. Load the effective config with `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/council_state.py effective-config`; if it fails, print the invalid-config message and stop.
...
- Otherwise show:
  ...
  - **Cycles:** `<cycles_done> / <max_cycles>` (cycles_done = output of `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/council_state.py history-count`; include its warning if invalid lines exist).
  ...
  - **Recent history:** the last 5 lines of `history.jsonl`, each as `#<cycle> <verdict> [sec:<security>] [ver:<verifier>] <commit> — <step>` (omit either tag for older history lines that lack the field).
