...
2. Load the effective config with `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/council_state.py effective-config` and resolve TARGET. If config is invalid, print the error and stop.
...
4. If the argument is a number, resolve its commit with `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/council_state.py lookup-commit --cycle <number>`. If the helper fails, print its message and stop.
